...
const getFriendsList = async (userId, runCleanup = false) => {
  try {
    // Optionally clean up orphaned relationships first
    if (runCleanup) {
      await cleanupOrphanedRelationships(userId);
    }
    ...
    const mutualFriends = await Promise.all(
      mutualFriendIds.map(async (friendId) => {
        try {
          ...
          // Check if user document exists
          if (!userDoc.exists()) {
            console.warn('Friend profile not found:', friendId);
          ...
          console.error('Error fetching mutual friend profile:', friendId, error);
          return null;
        }
      })
    );
...
const sendRideInvitation = async (rideId, inviterId, inviteeId) => {
  try {
    ...
    const invitationData = {
      ...
      inviter: {
        uid: inviterId,
        displayName: inviterData.profile?.displayName || inviterData.displayName || 'Unknown User',
    ...
    // Filter out undefined values from invitation data
    const cleanInvitationData = JSON.parse(JSON.stringify(invitationData, (key, value) => 
    ...
    const rideRef = doc(db, 'rides', rideId);
    await updateDoc(rideRef, {
      [`invitations.${inviteeId}`]: cleanInvitationData,
      updatedAt: serverTimestamp()
    });
    
    // Create notification for invitee
    const inviterName = inviterData.profile?.displayName || inviterData.displayName || 'Unknown User';
    const inviterPhotoURL = inviterData.profile?.photoURL || inviterData.photoURL || '';
    
    await createNotification({
      userId: inviteeId,
      type: 'ride-invitation',
      title: 'Ride Invitation',
      message: `${inviterName} invited you to join their ride`,
      metadata: {
        rideId: rideId,
        inviterId: inviterId,
